...
namespace DG.Tweening
{
    ...
    public class DOTweenAnimation : ABSAnimationComponent
    {
        ...
        public void CreateEvent(GameObject obj)
        {
            Debug.Log("create event");
            if (obj.GetComponent<DoTweenCustomDelay>() != null)
            {
                customDelayTime = obj.GetComponent<DoTweenCustomDelay>().delayTime;
...
//            }
        ...
        bool hasCustomDelay = false;
        float customDelayTime = 0;
        public void CompletedWithStep()
        {
            Debug.Log("---completed with step---");
            if(hasCustomDelay)
            {
                if (loops > 0)
                {
        ...
        public void Update()
        {
            if(hasCustomDelay)
            {
                if (tween != null)
                {
                    if (isPause && Time.time - pauseStartTime >= customDelayTime)
                    {
                        isPause = false;
